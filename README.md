...
- ✅ **Opening-balance carryover helpers** *(loader + projection shipped — orchestrator hook follow-up)* — `read_prior_period_closing_tbs(prior_dir, &[entity_codes])` walks `entities/*/period_close/trial_balances.json` and loads each entity's latest closing TB (picks latest by `(fiscal_year, fiscal_period)`).  `extract_opening_balances(&tb)` projects a closing TB onto its balance-sheet positions only (Asset / Liability / Equity + contra variants — P&L lines drop because they close to retained earnings).  `OpeningBalance` records carry the per-(entity, account) signed closing position.  **What's still pending**: the orchestrator-side hook that consumes these to seed period-N+1 generation — this PR ships the loading + projection layer that the next PR will wire into the runtime.
