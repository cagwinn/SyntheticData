...
- ✅ **Step-acquisition / divestiture rollforwards** *(model layer + manifest plumbing + per-shard emission shipped)* — two layers complete:
  1. Model: `OwnershipChangeType` enum (`ControlGained` / `ControlIncreased` / `ControlDecreased` / `ControlLost`) and `OwnershipChangeEvent` struct cover IFRS 3.42 (control gained → re-measure prior interest at FV with P&L gain/loss), IFRS 10.23 (changes within control → equity transaction, no P&L), and IFRS 10.B97 (control lost → deconsolidate, FV retained interest, gain/loss).  Helpers `p_and_l_gain_or_loss`, `triggers_pl_remeasurement`, `triggers_method_transition` derive the standards-correct treatment from event type + carrying / FV inputs.
  2. Manifest plumbing + per-shard emission (PR #155): `EntityConfig.ownership_changes: Vec<OwnershipChangeEntry>` declares mid-period events; the manifest builder lifts each entry into a full `OwnershipChangeEvent` with `entity_code` and `parent_entity_code` filled, and validates `effective_date` lies within the engagement period and ownership percents are in `[0, 1]`.  The shard runner writes `entities/{code}/intercompany/ownership_change_events.json` per entity (only when non-empty — backwards compatible byte-for-byte for v5.0–v5.1 archives).  The aggregate-phase NCI rollforward consumption (proper IFRS 3 / IFRS 10 treatment of equity transactions / re-measurement gains-losses) is the next follow-up.
