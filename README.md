...
- ✅ **Step-acquisition / divestiture rollforwards** *(model + manifest + shard + IFRS 10.23 equity-transaction + IFRS 3.42 mid-period control gained shipped)* — four layers complete:
  1. Model (PR #143): `OwnershipChangeType` (`ControlGained` / `ControlIncreased` / `ControlDecreased` / `ControlLost`) and `OwnershipChangeEvent` cover IFRS 3.42 (control gained → re-measure at FV), IFRS 10.23 (within-control → equity transaction), IFRS 10.B97 (control lost → deconsolidate).
  2. Manifest plumbing + per-shard emission (PR #155): `EntityConfig.ownership_changes` declares mid-period events; manifest builder validates + lifts; shard runner writes `entities/{code}/intercompany/ownership_change_events.json`.
  3. Aggregate-phase NCI rollforward consumption (PR #164): `compute_nci_rollforward` reads each entity's events and applies the IFRS 10.23 equity-transaction adjustment (`adjustment = -consideration_paid_or_received` for `ControlIncreased` / `ControlDecreased`).  `NciRollforward.equity_transaction_adjustments` records the signed delta.
  4. Mid-period `ControlGained` rollforward (this PR / v5.4): `NciInputs` now carries `period_start`; `compute_nci_rollforward` time-weights `period_net_income` / `period_oci` / `total_dividends_paid` by `(period_end - effective_date + 1) / (period_end - period_start + 1)` so the entity contributes only its post-acquisition fraction to consolidated profit.  Opening NCI seeds from the event's `acquisition_date_nci_fair_value` (or the input's, whichever is non-`None`).  `NciRollforward.pl_remeasurement_gain_or_loss` records the IFRS 3 § 42 P&L gain/loss = `previously_held_interest_fair_value - previously_held_interest_carrying`.  `ControlLost` mid-period (full deconsolidation with retained-interest re-measurement per IFRS 10.B97) is typed-error-rejected pending v5.5 follow-up.
- ✅ **Goodwill impairment testing** *(end-to-end shipped)* — three layers complete:
  1. Model: `CashGeneratingUnit`, `GoodwillAllocation`, `CguImpairmentTest`, `CguImpairmentResult` cover IAS 36 § 6 (CGU definition), § 80 (acquisition-date goodwill allocation), § 18 (recoverable = max FV-less-costs vs VIU), § 104 (impairment loss allocated first to goodwill, then pro-rata to other assets), and § 124 (no reversal of goodwill impairment).  `CguImpairmentTest::run()` is a pure function returning the standards-correct allocation.
  2. Manifest plumbing (PR #152): `CguConfig` → `CguPlan` validated + sorted; `cgu_plan` field added to `GroupConfig` and `GroupManifest` with `#[serde(default)]` for backwards compatibility.
  3. Aggregate-phase runner (PR #154): `AggregateOptions::cgu_test_inputs` carries per-period FV-less-costs / VIU estimates per CGU; `run_aggregate` joins these against the manifest plan, runs `CguImpairmentTest::run` per CGU, and emits `consolidated/cgu_impairment_tests.json` (only when test inputs are supplied — empty `cgu_test_inputs` is byte-identical to v5.0–v5.1 archives).
- ✅ **Hyperinflationary economies** *(end-to-end shipped — IAS 29 § 12 + IAS 21 § 42(b))* — four layers complete:
  1. Model: `HyperinflationStatus`, `GeneralPriceIndex`, `NetMonetaryPositionGainLoss` (PR #145).
  2. Manifest plumbing: `EntityConfig` → `ExpandedEntity` → `ManifestEntity.hyperinflation_status` (PR #148).
  3. IAS 21 § 42(b) closing-rate translation (PR #149): `translate_entity_tb_with_hyperinflation` applies closing-rate-for-all-items when `requires_restatement()` is true; non-hyperinflationary entities use the v5.0–v5.1 spot/average split byte-identically.
  4. IAS 29 § 12 indexed restatement (this PR): new `IndexedRestatement{opening_index, closing_index, average_index}` aggregate-layer type pre-multiplies each TB amount by its account-class restatement factor (`closing/opening` for non-monetary BS items + equity, `closing/average` for income-statement items, `1.0` for monetary BS items) before the closing-rate FX translation.  `translate_entity_tb_with_indexed_restatement` composes both standards: `local × restatement_factor × closing_rate`.  Existing `_with_hyperinflation` entrypoint delegates with `restatement = None`, preserving byte-identical behaviour for engagements that don't supply price-index data.
...
- ✅ **Multi-period consolidation chain helper** *(shipped)* — `run_aggregate_chain(Vec<PeriodSpec>)` runs `run_aggregate` N times in sequence, automatically threading each period's `out_dir` as the next period's `prior_period_aggregate` so opening NCI / CTA / equity-method / suppressed-loss values stitch across periods without manual path-weaving.  Period 0 keeps any caller-supplied `prior_period_aggregate` (lets engagements seed from an external archive); periods 1..N have it overridden to the previous period's `out_dir`.
- ✅ **Multi-period orchestrator chain helper** *(shipped)* — `generate_standalone_chain(base_cfg, Vec<PeriodChainSpec>, base_out_dir, opts)` extends the chain pattern to the **full pipeline** (manifest + shards + aggregate per period), not just the aggregate phase.  Each `PeriodChainSpec { period, out_subdir }` overrides `base_cfg.period` and routes outputs to `base_out_dir/{out_subdir}/`.  The aggregate-phase prior-period plumbing is auto-threaded so opening NCI / CTA / equity-method / suppressed-loss values flow forward across periods.  Validation rejects empty period lists and duplicate `out_subdir` values up front.
- ✅ **Opening-balance carryover** *(end-to-end shipped — multi-period TB continuity)* — four layers:
- ✅ **AuditMethodology v0.14 KYC blueprints** *(6 workflows shipped)* — port from the AuditMethodology repo's `docs/blueprints/kyc/`.  New module `crates/datasynth-audit-fsm::kyc_blueprint` carries `KycBlueprint` / `KycProcedure` / `KycStep` types matching the methodology v0.14 KYC schema (separate from the audit blueprint schema since KYC uses `obligation_refs` like `obl:eu_mica:60:1` rather than `isa_refs`).  Six YAMLs embedded via `include_str!`:
  - **`kyc_casp_onboarding`** — Crypto-asset service provider onboarding (MiCA + TFR + AMLR 2024/1624)
...
- ✅ **Opening-balance carryover** *(loader + projection + orchestrator hook shipped)* — three layers:
  1. Loader (PR #165): `read_prior_period_closing_tbs(prior_dir, &[entity_codes])` walks `entities/*/period_close/trial_balances.json` and loads each entity's latest closing TB (picks latest by `(fiscal_year, fiscal_period)`).
  2. Projection (PR #165): `extract_opening_balances(&tb)` projects a closing TB onto its balance-sheet positions only (Asset / Liability / Equity + contra variants — P&L lines drop because they close to retained earnings); group-side `OpeningBalance` carrier records the per-(entity, account) signed closing position.
  3. Orchestrator hook (PR #166): `EntityOpeningBalance` carrier in `datasynth-core::models::balance` + `ShardContext.opening_balances: Vec<EntityOpeningBalance>` field + `EnhancedOrchestrator::phase_opening_balances` consumption (builds the period's `GeneratedOpeningBalance` directly from carryovers when non-empty, falls through to the industry-mix generator otherwise).
  4. Chain-helper integration (PR #168): `StandaloneOptions.entity_opening_balances` plus a new `run_shard_with_opening_balances` entrypoint thread carryovers from chain helper through standalone → shard runner → orchestrator.  `generate_standalone_chain` now auto-loads each entity's prior-period closing TB between periods N and N+1 — no caller plumbing required.  Single-period engagements and empty carryovers preserve v5.0–v5.2 byte-identical behaviour.
- ✅ **Jurisdictional audit overlays** *(7 overlays, 39 procedures shipped)* — port from the AuditMethodology repo (v0.14): PCAOB (US), EU CSRD listed (eu_listed_first_wave), UK FRC (uk_listed), ASIC (au_listed), JFSA (jp_listed), ACRA (sg_listed), HKICPA (hk_listed).  Each overlay declares ISO-3166 jurisdiction codes + registrant types + procedures with statute / standard citation refs.  `crates/datasynth-audit-fsm::jurisdictional_overlay::resolve_overlays(jurisdiction, registrant_type)` applies AND-logic on `(jurisdiction × registrant_type)` to determine which overlays fire — empty `registrant_types` is treated as wildcard (PCAOB applies to any US registrant).  Multi-jurisdictional (dual-listed) engagements call resolve once per jurisdiction and union the results.
- ✅ **AuditMethodology v0.14 blueprints** *(ISA 600 Revised + CSRD limited assurance shipped)* — port from the AuditMethodology repo's `docs/blueprints/{group_audit,csrd}/`.  New module `crates/datasynth-audit-fsm::methodology_blueprint` carries `MethodologyBlueprint` / `MethodologyPhase` / `MethodologyProcedure` / `MethodologyStep` types matching the methodology v0.14 schema.  Two YAMLs embedded via `include_str!`:
  - **`iso_600_revised`** — ISA 600 (Revised, Dec 2022) covering all requirement paragraphs across 6 phases (acceptance + continuance → planning & scoping → component communication → evidence & misstatements → subsequent events & review → documentation & communication).  Pairs naturally with the v5.0 `datasynth-group` group-audit simulation crate.
  - **`csrd_limited_assurance`** — CSRD limited-assurance engagement on ESRS-formatted sustainability statements with 100% topical standard coverage (E1–E5 environmental, S1–S4 social, G1 governance) per Directive (EU) 2022/2464 + IAASB ISSA 5000.
