...
- ✅ **Opening-balance carryover** *(loader + projection + orchestrator hook shipped)* — three layers:
  ...
  3. Orchestrator hook (this PR): new `EntityOpeningBalance` carrier in `datasynth-core::models::balance` (carries `account_code` / `account_type` / `debit` / `credit`); `ShardContext.opening_balances: Vec<EntityOpeningBalance>` lets the shard runner thread per-entity carry-forwards into the orchestrator; `EnhancedOrchestrator::phase_opening_balances` builds the period's `GeneratedOpeningBalance` directly from carryovers (totals classified by `AccountType`) instead of calling `OpeningBalanceGenerator` when non-empty.  Empty carryovers (the v5.0–v5.2 default) fall through to the generator path byte-identically.  Chain-helper integration that auto-threads the loaders into period N+1's `ShardContext` is the natural next PR.
