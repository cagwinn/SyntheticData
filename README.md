...
- ✅ **Hyperinflationary economies** *(end-to-end shipped — closing-rate path)* — three layers complete:
  1. Model: `HyperinflationStatus`, `GeneralPriceIndex`, `IndexedRestatement`, `NetMonetaryPositionGainLoss` (PR #145).
  2. Manifest plumbing: `EntityConfig` → `ExpandedEntity` → `ManifestEntity.hyperinflation_status` (PR #148).
  3. Translation pipeline (this PR): `translate_entity_tb_with_hyperinflation` applies the IAS 21 § 42(b) closing-rate-for-all-items override when `requires_restatement()` is true.  `run_aggregate` looks up the manifest flag per entity and routes to the right path.  Non-hyperinflationary entities (the default) use the v5.0–v5.1 spot/average split byte-identically.

  Still pending for full IAS 29 compliance: the explicit IAS 29 § 12 indexation of non-monetary items prior to translation (the `IndexedRestatement` helper exists at the model layer but the aggregate phase doesn't yet apply it to the entity TB).  The closing-rate translation in this PR is the IAS 21 § 42(b) half — sufficient for many engagements but pure-play IAS 29 needs the indexation step too.
