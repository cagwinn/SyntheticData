...
- ✅ **Step-acquisition / divestiture rollforwards** *(model + manifest + shard + IFRS 10.23 equity-transaction rollforward shipped)* — three layers complete:
  ...
  3. Aggregate-phase NCI rollforward consumption (this PR): `compute_nci_rollforward` reads each entity's events and applies the IFRS 10.23 equity-transaction adjustment (`adjustment = -consideration_paid_or_received` for `ControlIncreased` / `ControlDecreased`).  `NciRollforward.equity_transaction_adjustments` records the signed delta.  `ControlGained` / `ControlLost` mid-period (full consolidation entry / exit with pro-rated profit attribution) are typed-error-rejected with a v5.4 follow-up pointer — period-1 acquisitions still use `acquisition_date_nci_fair_value` from PR #141.
