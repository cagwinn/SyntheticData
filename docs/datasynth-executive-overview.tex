...
\fancyhead[R]{\small\textcolor{midgray}{v2.2.0}}
...
\begin{flushleft}
  ...
  {\normalsize\textcolor{white!50}{Rust core \textbullet{} Python 1.10 \textbullet{} REST/gRPC API \textbullet{} Audit FSM Engine}}
\end{flushleft}
...
\begin{flushleft}
  ...
  {\Large\textcolor{brand}{\textbf{Version 2.2.0}}}\\[1.5em]
  \textcolor{midgray}{%
...
\begin{flushleft}
  \textcolor{midgray}{\small
    Built with Rust \& Python \textbullet{} 20 modular crates \textbullet{} 200\,K+ entries/sec\\[4pt]
...
\subsection{Financial Coherence Engine}

\textit{New in v2.2.0} --- a cross-module coherence layer that connects
manufacturing, treasury, tax, HR, and reporting generators into a unified
financial data fabric. Every journal entry traces back to an originating
business event, and every financial statement line item reconciles to the
underlying GL.

\subsubsection{Manufacturing Cost Flow}

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{WIP\,$\to$\,FG\,$\to$\,COGS cycle} --- multi-stage cost
        accumulation with standard cost variance journal entries for material
        price, labor rate, and overhead volume variances.
  \item \textbf{Warranty provisions} --- IAS\,37/ASC\,450 provisions derived
        from quality inspection failure rates, with automated accrual and
        release entries.
  \item \textbf{COGS on delivery} --- cost of goods sold recognized at shipment,
        linked to production unit costs for full cost traceability.
\end{itemize}

\subsubsection{Treasury \& Debt Accounting}

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Effective interest method} --- debt interest accrual journal
        entries computed from amortized cost and contractual terms.
  \item \textbf{Hedge mark-to-market} --- ASC\,815/IFRS\,9 routing with cash
        flow hedge gains/losses posted to OCI and fair value hedge adjustments
        posted to P\&L.
  \item \textbf{Covenant compliance} --- real-time evaluation against trial
        balance ratios (debt/equity, interest coverage, current ratio).
  \item \textbf{Cash pool sweeps} --- physical sweep intercompany entries with
        proper elimination on consolidation.
\end{itemize}

\subsubsection{Tax Integration}

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Income tax provisions} --- derived from actual pre-tax income
        by netting revenue and expense GL accounts, ensuring provision-to-GL
        reconciliation.
  \item \textbf{Transaction-level VAT/GST} --- input and output tax journal
        entries generated from source documents (vendor invoices, customer
        invoices, payments).
  \item \textbf{Document-date-aware posting} --- tax entries posted on the
        source document date, avoiding artificial period-end clustering.
\end{itemize}

\subsubsection{Payroll $\leftarrow$ HR Events}

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Change-history-driven payroll} --- salary adjustments consume
        \texttt{EmployeeChangeHistory} records with mid-period proration for
        hires, promotions, and terminations.
  \item \textbf{Variance traceability} --- payroll period-over-period variances
        are traceable to specific HR events, supporting audit walkthroughs of
        compensation changes.
\end{itemize}

\subsubsection{Full Financial Statement Package}

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Enhanced cash flow statement} --- operating, investing, and
        financing items sourced from all active modules (treasury, tax,
        payroll, manufacturing, project accounting).
  \item \textbf{IFRS vs US GAAP classification} --- interest paid/received
        classified per framework requirements (operating vs.\ financing).
  \item \textbf{Segment reporting} --- operating segments derived from actual
        entity-level GL data with segment-to-consolidated reconciliation.
  \item \textbf{Data-backed notes} --- financial statement notes for inventory,
        debt maturities, hedge positions, and provisions with traceable source
        references to underlying records.
  \item \textbf{Dividend lifecycle} --- declaration, accrual, and payment
        journal entries with proper equity and cash flow classification.
  \item \textbf{XBRL 2.1 export} --- instance document generation with
        US GAAP and IFRS taxonomy mapping for regulatory filing.
\end{itemize}

\subsubsection{Capstone Coherence Validators}

\begin{itemize}[leftmargin=1.2em, itemsep=3pt]
  \item \textbf{Cash flow reconciliation} --- validates that opening cash plus
        net operating/investing/financing flows equals the closing GL cash balance.
  \item \textbf{Equity rollforward} --- confirms opening equity plus net income
        plus OCI minus dividends equals closing equity.
  \item \textbf{Segment reconciliation} --- verifies that segment totals
        reconcile to consolidated financial statement figures.
  \item \textbf{Trial balance master proof} --- asserts that the sum of all
        posted journal entries equals the closing trial balance for every account.
\end{itemize}
...
\begin{tikzpicture}[
  ...
  \node[layer, minimum width=3.5cm, right=0.6cm of server, draw=midgray, text=midgray, fill=white] (ui) {datasynth-ui\,\scriptsize(deprecated)};
...
Streaming Output &
  CSV, JSON, NDJSON, Apache Parquet (15-column Arrow schema, Zstd
  compression), and XBRL\,2.1 instance documents (US GAAP \& IFRS taxonomies).
  Streaming sinks with backpressure strategies
...
Desktop UI &
  \textit{Deprecated in v2.2.0.} Previously a Tauri + SvelteKit application;
  superseded by REST/gRPC API and Python wrapper workflows. \\
Plugin SDK &
